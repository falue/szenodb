...
<!--  -->
  <form @submit.prevent="dataMode === 'new' ? $emit('new', {...data, 'note': user.notes}) : $emit('edit', {'newData': {...data, 'note': user.notes}, 'oldData': oldData});">
    ...
    <v-text-field style="" :style="addDetails ? 'display:inline-block; width:40%;' : ''" class="mr-2" type="text" v-model.trim="data.title" :rules="requiered">
      <template v-slot:label>Company/Title<span v-if="data.title.length == 0" class="red--text">*</span></template>
    </v-text-field>
    <v-text-field v-if="addDetails" label="Contact person" style="width:55%; display:inline-block" type="text" v-model.trim="data.name"></v-text-field>
    <v-textarea auto-grow rows="3" hint="Resources or services of this business. Will auto translate to EN/DE/FR/IT." type="text" :rules="requiered" v-model.trim="data.resources">
      <template v-slot:label>Resources<span v-if="data.resources.length == 0" class="red--text">*</span></template>
    </v-textarea>
    <v-textarea auto-grow rows="3" label="Opinions" hint="Delivery speed, what to look out for, etc. Will auto translate to EN/DE/FR/IT." type="text" v-model.trim="data.info"></v-textarea>
    <v-textarea auto-grow rows="1" label="Private note" hint="These notes do not show up to other users." type="text" v-model.trim="user.notes[data.id]"></v-textarea>
    <v-text-field v-if="addDetails" label="Address" hint="Can be typed address, google share link or general region." type="text" v-model.trim="data.address"></v-text-field>
    <v-text-field v-if="addDetails" label="Telephone" style="width:40%; display:inline-block" class="mr-2" type="text" v-model.trim="data.tel"></v-text-field>
    <v-text-field v-if="addDetails" label="Email" style="width:55%; display:inline-block" type="text" v-model.trim="data.email"></v-text-field>
    <v-text-field label="Website" type="text" v-model.trim="data.web"></v-text-field>
